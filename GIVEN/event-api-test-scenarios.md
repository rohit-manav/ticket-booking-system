...
11. **Activate event (without seats)** — `PATCH /api/v1/events/{eventId}/status`
   - Expected: `409 EventHasNoSeats`.
12. **Activate event (INACTIVE → ACTIVE)** — `PATCH /api/v1/admin/events/{eventId}/status`
   - Expected: `200 OK`.
13. **Activate event (already ACTIVE)** — `PATCH /api/v1/admin/events/{eventId}/status`
   - Expected: `409 EventAlreadyActive`.
14. **Deactivate event (ACTIVE → INACTIVE)** — `PATCH /api/v1/admin/events/{eventId}/status`
    - Expected: `200 OK`.
15. **Deactivate event (already INACTIVE)** — `PATCH /api/v1/admin/events/{eventId}/status`
    - Expected: `409 EventAlreadyInactive`.
16. **Update status (invalid status)** — `PATCH /api/v1/admin/events/{eventId}/status`
    - Expected: `400 ValidationFailed` or `400 HttpMessageNotReadable`.
17. **Update status (non-existent ID)** — `PATCH /api/v1/admin/events/{eventId}/status`
    - Expected: `404 ResourceNotFound`.
18. **Delete event (happy path)** — `DELETE /api/v1/admin/events/{eventId}`
    - Expected: `204 No Content` (soft delete).
19. **Delete event (non-existent ID)** — `DELETE /api/v1/admin/events/{eventId}`
    - Expected: `404 ResourceNotFound`.
20. **Delete event (with confirmed bookings)** — `DELETE /api/v1/admin/events/{eventId}`
    - Expected: `204 No Content` (soft delete still allowed).

## Event Browsing (Admin or Customer)
21. **List active events (default pagination)** — `GET /api/v1/events`
    - Expected: `200 OK`, `limit=25`, `offset=0`, `totalCount` present.
22. **List active events (custom pagination)** — `GET /api/v1/events?limit=5&offset=0`
    - Expected: `200 OK`, `limit=5`.
23. **List active events (page 2)** — `GET /api/v1/events?limit=5&offset=5`
    - Expected: `200 OK`, `offset=5`.
24. **List active events (sort by name)** — `GET /api/v1/events?sort=name,asc`
    - Expected: `200 OK`, sorted A→Z.
25. **List active events (sort by eventDateTime desc)** — `GET /api/v1/events?sort=eventDateTime,desc`
    - Expected: `200 OK`, latest first.
26. **List active events (limit clamped)** — `GET /api/v1/events?limit=9999`
    - Expected: `200 OK`, `limit=100`.
27. **List active events (empty result)** — `GET /api/v1/events?offset=100000`
    - Expected: `200 OK`, empty `items`.
28. **Get event by ID (ACTIVE)** — `GET /api/v1/events/{eventId}`
    - Expected: `200 OK`.
29. **Get event by ID (INACTIVE)** — `GET /api/v1/events/{eventId}`
    - Expected: `404 ResourceNotFound`.
30. **Get event by ID (non-existent)** — `GET /api/v1/events/{eventId}`
    - Expected: `404 ResourceNotFound`.
